...
@app.route('/api/servers/<int:server_id>/inbounds', methods=['GET'])
@login_required
def get_server_inbounds(server_id):
    server = Server.query.get_or_404(server_id)
    session_obj, error = get_xui_session(server)
    ...
    return jsonify({"inbounds": processed_inbounds})
...
def add_client():
    ...
    start_after_use = data.get('start_after_use', False)
    
    if not all([server_id, inbound_id, email]):
    ...
    if not inbound:
        return jsonify({"success": False, "error": "Inbound not found"})
    
    settings = json.loads(inbound.get('settings', '{}'))
    clients = settings.get('clients', [])
    
    if any(c.get('email') == email for c in clients):
        return jsonify({"success": False, "error": f"Email '{email}' already exists in this inbound"})
    
    client_data = {
        'email': email,
        'enable': True,
        'limitIp': 0,
        'totalGB': int(limit_gb * 1024 * 1024 * 1024) if limit_gb > 0 else 0,
        'expiryTime': 0
    }
    
    if password:
        if server.panel_type == 'trojan':
            client_data['password'] = password
        else:
            client_data['id'] = password
    
    if days > 0:
        if start_after_use:
            client_data['expiryTime'] = -1 * (days * 86400000)
        else:
            client_data['expiryTime'] = int((datetime.utcnow().timestamp() + days * 86400) * 1000)
    
    try:
        if server.panel_type == 'sanaei':
            url = f"{server.host}/panel/api/inbounds/{inbound_id}/addClient"
        else:
            url = f"{server.host}/xui/API/inbounds/addClient"
        
        response = session_obj.post(
            url,
            json={'clientId': '', 'settings': json.dumps(client_data)},
            verify=False,
            timeout=10
        )
        
        if response.status_code not in [200, 201]:
            return jsonify({"success": False, "error": f"Failed to add client: {response.text}"})
